...
6. **Toolbars & Navigation**: Keep navigation bars standard. Prefer `.navigationTitle` + `.toolbarTitleMenu` for menus and `ToolbarItem` placements like `.cancellationAction` / `.confirmationAction`. Avoid custom principal views, hidden toolbar backgrounds, or bespoke toolbar containers.

7. **Primary Actions**: If a branded primary action is required in a toolbar, use a semantic button primitive (e.g., `ToolbarPrimaryButton`) placed in a standard toolbar slot. Do not wrap it in extra glass/surface backgrounds.

8. **Consistent Design**: Maintain consistent shapes and styles across your app for a cohesive look and feel.
