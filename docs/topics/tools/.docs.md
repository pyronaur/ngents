...
summary: Portable CLI tool catalog with approved workflows and baseline commands for tools used across repos.
---
...
- It uses the HIG Doctor bundle from the external docs collection at `docs topic app hig-doctor`; no separate `hig-doctor` executable is required on `PATH`.
