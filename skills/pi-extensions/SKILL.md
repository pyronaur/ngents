---
name: pi-extensions
description: Search and suggest Pi packages, especially extensions, from a local cache of the pi.dev package gallery and package registry results. Use when explicitly invoked to find Pi extensions/packages by capability, compare install candidates, or list packages by type.
...
- Use `node scripts/package-metadata.mjs <package...>` for package metadata and repository/homepage links.
- Do not use `npm view`, `npx`, or `pnpm view`; package-manager entrypoints are gated on this machine.
- Do not use `bun pm view`; it requires a project `package.json` in the current directory.
- `pi install npm:<package>` is Pi's package-source syntax, not a command to inspect packages.

## Source facts

- `pi.dev/packages` has no browser-visible JSON API for the catalog.
- The gallery is registry packages tagged `pi-package`.
- Registry search API: `https://registry.npmjs.org/-/v1/search?text=keywords:pi-package&size=250&from=0`.
...
- Use registry cache when package descriptions, keywords, authors, publisher, version, or links are enough.
...
- `scripts/package-metadata.mjs [--json] <package...>`: fetch package metadata directly from the registry without invoking npm/pnpm/Bun package-manager commands.
